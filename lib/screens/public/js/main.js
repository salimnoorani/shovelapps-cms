...
(function(listPlugins){
	...
	var checkIframe = function(){
		...
		var final_html = html.replace(new RegExp(/\n/g), "");
		 ...
		 $(".iframe-demo").attr("src", "data:text/html;charset=utf-8," + final_html);
	};
	...
	var addPlugin = window.ShovelApps.Plugins.addPlugin = function(id, target, state){
	...
	var loadPluginsStates = window.ShovelApps.Plugins.loadPluginsStates = function(states)
	{
		try
		{
			for(var i in states)
			{
				...
				addPlugin(plugin.id, false, plugin.state);
			}
			return true;
		}
		catch(err)
