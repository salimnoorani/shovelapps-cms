...
(function(listPlugins){
	...
	var checkIframe = function(){
		...
		for(var i in css)
		{
			...
			{
				for(var j in css[i])
					$('<link>').appendTo(head).attr({type : 'text/css', rel : 'stylesheet', async: true}).attr('href', css[i][j]);
			}
			else if(css[i].trim() != "")
			{
	...
	var addPlugin = window.addPlugin = function(id, target, state){
		...
		try
		{
			...
			require([data.main.js], function(p){
				var plugin = new p(), plugin_template = plugin.getTemplate(), fadeTime = 300;

				template.data("plugin", plugin).find(".content:first").append(plugin_template);
