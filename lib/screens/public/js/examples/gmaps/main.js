define(function(require){
	...
	gmaps.prototype.getHtml = function()
	{
		...
		script.push('var resize = function(){if(typeof $ == "undefined") { setTimeout(resize, 6000); console.log("no jquery yet"); return;};console.log("RESIZE MAP"); $("#' + map_var +'").width("'+this.input_width.val()+'").height("'+this.input_height.val()+'");setTimeout(function(){google.maps.event.trigger(' + map_var +', "resize");}, 2000); var interval = setInterval(function(){ if($("#' + map_var +'").is(":visible")){ clearInterval(interval); google.maps.event.trigger($("#' + map_var +'").get(0), "resize"); ' +map_var+'.setCenter(' + map_settings.lat +', ' + map_settings.lng +'); }}, 1000); }; ');
		script.push("google.maps.event.addDomListener(window, 'load', function(){ setTimeout(resize, 2000);});");
